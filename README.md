...
```bash
# Generate CSV templates with examples
python scripts/csv_tools.py multi-template ./my_data

# Convert XML to multiple CSV files
python scripts/csv_tools.py xml-to-csv-folder data.xml ./csv_output

# Convert CSV folder back to XML (validation runs by default)
python scripts/csv_tools.py csv-folder-to-xml ./csv_output output.xml

# Convert without validation
python scripts/csv_tools.py csv-folder-to-xml ./csv_output output.xml --no-validate
```
...
Before converting CSV files to XML, you can validate them using the built-in `CsvFolderValidator`:

```python
from okfn_iati import CsvFolderValidator, ValidationLevel

validator = CsvFolderValidator()
result = validator.validate_folder("./my_iati_data")
...
Validation checks include required fields, date formats, codelist values, cross-file foreign key consistency, and percentage totals.
